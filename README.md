...
- **FP8 KV cache** (`KV8=1`) — the MLA latent rows are stored as fp8 e4m3 with a per-row scale: 3.9× less latent-KV RAM (584 vs 2304 B/token/layer-row; at 256k the latent pool drops ~47.7 → ~12.1 GB, ~13 GB counting the f32 DSA indexer keys that both modes keep), `.coli_kv` shrinks to ~46 KB/token (v2 format; a v1 f32 save is quantized on resume and the file is rewritten as v2 at the first append after a completed turn), and the RAM budget planner sees the smaller pool, so multi-slot serving stops demoting experts to disk. DeepSeek-V3-class KV quantization noise; covered on the CPU and CUDA attention paths (`COLI_CUDA_ATTN=1` uses the fp8 kernels — per-call KV uploads are ~¼ the f32 bytes, and the default device-resident shadow uploads each row only once; the fp8 GPU path caps at 1M tokens, beyond which attention falls back to CPU); auto-disabled under `COLI_METAL`.
