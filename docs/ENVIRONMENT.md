...
| `KV_SHADOW` | auto (on iff `KV_SLOTS`=1) | KV8-only: keep a device-resident fp8 copy of the KV so CUDA decode attention reads VRAM instead of re-uploading history. Auto-off for multi-slot serving, where ragged decode runs on CPU and the shadow would spend ~`CTX×584 B`/layer of VRAM for nothing. The `CUDA_EXPERT_GB=auto` budget accounts for the shadow when it's on. `0`/`1` force. |
