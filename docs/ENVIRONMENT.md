...
| `KV8` | `0` (off) | Store the MLA latent KV cache in fp8 e4m3 with a per-row scale: ~3.9× less KV RAM, and `.coli_kv` shrinks ~4× (saved as the v2 format; f32 v1 files are quantized on resume and rewritten). Adds DeepSeek-V3-class KV quantization noise to attention. Covered on the CPU and CUDA attention paths; under `COLI_CUDA_ATTN=1` the fp8 KV lives in a device-resident shadow with **no context-length cap** (split-T kernels beyond the old 4096/8192 limits; DSA-selected decode uses a gather kernel and respects the selection, unlike the dense-only f32 GPU path) — measured 11–17× faster than the CPU fallback at 16k–262k tokens. Auto-disabled under `COLI_METAL`; forces `COLI_CUDA_PIPE=0`. |
| `KV_SHADOW` | auto (on under KV8+CUDA) | KV8-only: keep a device-resident fp8 copy of the KV so CUDA attention reads VRAM instead of re-uploading history. Auto is simply ON when KV8 runs with CUDA: chunked prefill needs it (without it every chunk re-uploads the whole fp8 history per layer — ~170 GB of PCIe measured on a 124k prompt), and ragged multi-slot decode uses it for the currently-bound slot too. Costs ~`CTX×584 B`/layer of VRAM (1.5 GB-free floor at alloc, fail-soft to per-call upload); the shadow is shared across slots, so interleaved multi-slot traffic re-uploads a slot's history after each slot switch (per-slot shadows are the known follow-up). The `CUDA_EXPERT_GB=auto` budget accounts for the shadow when it's on. `0`/`1` force. |
