...
static id<MTLComputePipelineState> g_a_tq_enc, g_a_tq_deq;              // KV_TQ PolarQuant attention
...
extern "C" int coli_metal_init(void) {
  ...
  @autoreleasepool {
    ...
    NSError *err = nil;
    // fast-math OFF: the fp8 RNE encoder (a_fp8enc) must match coli_fp8_enc bit-for-bit,
    // so rint/frexp/ldexp at rounding ties may not be reassociated. Costs nothing on the
    // dot-product kernels (they only get MORE accurate).
    MTLCompileOptions *copts = [MTLCompileOptions new];
    if (@available(macOS 15.0, *)) copts.mathMode = MTLMathModeSafe;   // exact rint/frexp for fp8
    else copts.fastMathEnabled = NO;
    ...
    g_a_fp8enc=P("a_fp8enc"); g_a_score8=P("a_score8"); g_a_clat8=P("a_clat8");
    g_a_tq_enc=P("a_tq_enc"); g_a_tq_deq=P("a_tq_deq");
    ...
    if(!g_a_tq_enc||!g_a_tq_deq){ fprintf(stderr,"[metal] tq attn pipelines failed\n"); g_dev=nil; return 0; }
...
static id<MTLBuffer> aLstg_,aRstg_; static size_t aLstg_cap,aRstg_cap;        // KV_TQ consumer f32 stage (T rows)
...
static bool encode_attention(id<MTLComputeCommandEncoder> e, const AttnW *W,
    ...
    // KV_TQ: reconstruct ALL T rows to the f32 stage, then the f32 score/clat run on them.
    if(qmode==2){
