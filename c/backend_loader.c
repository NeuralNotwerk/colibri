...
typedef int (*fn_scratch_failed)(void);
...
static struct {
    ...
    fn_scratch_failed scratch_failed;
...
static int coli_cuda_load(void){
    ...
    RESOLVE(scratch_failed, fn_scratch_failed)
...
int coli_cuda_scratch_failed(void){
    if(!g_cuda.available){ return 0; }
    return g_cuda.scratch_failed();
}
